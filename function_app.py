...
def extrair_itjobs(chave):
    ...
    for pagina in range(1, 3):  # Vai buscar as primeiras 2 páginas
...
# --- TRABALHADOR 2: Jooble (Versão com Detetor de Erros) ---
def extrair_jooble(chave):
    ...
    # Usar o endereço global recomendado pela documentação do Jooble
    url = f"https://jooble.org/api/{chave}"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.info(f"O Jooble encontrou {len(vagas)} vagas para descarregar.")
            for item in vagas:
                vaga = {
            ...
            # Imprime no log o erro exato que o site nos devolveu
            logging.error(f"Erro no Jooble! Código: {response.status_code} | Resposta do site: {response.text}")
            
    except Exception as e:
        logging.error(f"Erro Fatal ao extrair do Jooble: {e}")
        
    return vagas_formatadas


# --- TRABALHADOR 3: Remotive (Sem Chave) ---
...
def ScraperOportunia(myTimer: func.TimerRequest) -> None:
    ...
    if not LIGACAO:
        logging.error("ERRO FATAL: Falta a ligação ao Cosmos DB no Azure!")
        return
